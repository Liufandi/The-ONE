...
public abstract class Connection {
	...
	public Connection(DTNHost fromNode, NetworkInterface fromInterface, 
		...
		/**
		 * per half hour statistics
		 * 相遇次数统计 
		 */		
		int time = SimClock.getIntTime()/ 1800;
		int day=SimClock.getIntTime()/86400;
				
		while (Math.floor(time) > currentHalfHour) {			
			currentHalfHour++;
		} 
		currentHalfHour=currentHalfHour-48*day;
		System.out.println("day:  "+day+"           currentHalfHour:   "+currentHalfHour);
		this.fromNode.PerThrityMinuteContact[day][currentHalfHour]++;
		this.toNode.PerThrityMinuteContact[day][currentHalfHour]++;
		//System.out.println("现在的currentHalfHour：   "+currentHalfHour);
